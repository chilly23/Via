...
let colorIndex = 0;
// Use a Map object instead of invalid variable name with hyphen
let existingRoutes = new Map();
...
function updateMapWithRoute(routeData) {
    ...
    // Add a simple line between source and destination
    // let newRoute = L.polyline([sourceLatLng, destinationLatLng], {
    //     color: routeColor,
    //     weight: 5,
    //     opacity: 0.8
    // }).addTo(routesLayer).bindPopup(`Route shared by ${userId.substring(0, 8)}`);
...
function updateRoute() {
    ...
    Promise.all([getCoordinates(sourceName), getCoordinates(destinationName)])
        .then(([sourceCoords, destinationCoords]) => {
            ...
            const sourceLatLng = L.latLng(sourceCoords[0], sourceCoords[1]);
            ...
            saveSearch('source', sourceName);
            saveSearch('destination', destinationName);

            // Broadcast route to other users
